...
def get_image(image_path, input_height, input_width,
              ...
              crop=True):
  image = load_webp(image_path)
  #image_transform = transform(image, input_height, input_width, resize_height, resize_width, crop)
...
def merge(images, size):
  ...
  if (images.shape[3] in (3,4)):
    c = images.shape[3]
    img = np.zeros((h * min(4, size[0]), w * min(4, size[1]), c))
...
def center_crop(x, crop_h, crop_w,
  ...
  return np.array(Image.fromarray(np.array(x)[j:j+crop_h, i:i+crop_w]).resize(size=(resize_h, resize_w)))
  #return scipy.misc.imresize(
  #    x[j:j+crop_h, i:i+crop_w], [resize_h, resize_w])

def transform(image, input_height, input_width,
              resize_height=64, resize_width=64, crop=False): # original: crop=True
  if crop:
    #print("crop!!!!!")
