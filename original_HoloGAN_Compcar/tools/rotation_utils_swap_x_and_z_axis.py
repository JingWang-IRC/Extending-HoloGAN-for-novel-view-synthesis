...
def tf_interpolate(voxel, x, y, z, out_size):
    ...
    x_s_flat, y_s_flat, z_s_flat are gridnet
    voxel_array may be feature maps
    ...
    out_height = out_size[1] # new_size
    out_width = out_size[2]  # new_size
    out_depth = out_size[3]  # new_size
    out_channel = out_size[4]

    zero = tf.zeros([], dtype='int32') # 0 dimension zero tensor
    ...
    x0 = tf.clip_by_value(x0, zero, max_x)  # if transformed coordinates exceed the size of the voxel, clip them
    x1 = tf.clip_by_value(x1, zero, max_x)  # if the size of grid is much bigger than the voxel, it might cause some problem
    ...
    #tf.range(batch_size) * width * height * depth : Element to repeat. Each element in the list is incremented by width*height*depth amount
    # out_height * out_width * out_depth: n of repeat. Create chunks of out_height*out_width*out_depth length with the same value created by tf.rage(batch_size) *width*height*depth
    ...
    base_z0 = base + z0 * width * height   # both base and z0 have the shape of (batch_size * out_height * out_width * out_depth,)
    ...
    Ia = tf.gather(voxel_flat, idx_a) # tf_gather: slice the matrix along axis=0(default, can be set to other axis) and stick some of them together(idx_a in this case)
...
def tf_voxel_meshgrid(height, width, depth, homogeneous = False):
    with tf.variable_scope('voxel_meshgrid'):
        ...
        # I do not know why you swapped x and z, even though you did not swap them, this still works!!! (need to be tested)
...
def tf_rotation_around_grid_centroid(view_params, shapenet_viewer = False):
    ...
    batch_size = tf.shape(view_params)[0]  # shape: (batch_size * 6) 6: azimuth, elevation, scale, translation_x ~ z
    ...
    #Batch Rotation Z matrixes　　Why you rotates the matrix around Z-axis? It is different to your annotation before
...
def generate_random_rotation_translation(batch_size,
    ...
    # in fact, the author made a mistake here, she mistakenly swap scale_low=1.0 and scale_high=1.0 to back position
    # this mistake pass 0 and 0 in scale = float(np.random.uniform(scale_low, scale_high)) which caused problem
