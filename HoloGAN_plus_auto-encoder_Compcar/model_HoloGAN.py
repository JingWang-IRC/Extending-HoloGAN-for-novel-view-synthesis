...
class HoloGAN(object):
  ...
  def build_HoloGAN(self):
    ...
    if str.lower(str(cfg["style_disc"])) == "true":      # use style distance
        ...
        self.D, self.D_logits, self.Q_c, self.camera, self.d_h1_r, self.d_h2_r, self.d_h3_r, self.d_h4_r, self.d_h3_original, self.d_h4_original = dis_func(self.inputs, cont_dim=cfg['z_dim'], reuse=False)
        self.D_, self.D_logits_, self.Q_c_given_x, self.camera_given_x, self.d_h1_f, self.d_h2_f, self.d_h3_f, self.d_h4_f, _, _ = dis_func(self.G, cont_dim=cfg['z_dim'], reuse=True)
    ...
    # the following lines are about viewpoint, which is a part of the new added auto-encoder
    # the value in self.view_in is radian (except scaling), while the output of discriminator (encoder) ranges from 0~1
    # to change radian to 0~1, the next several lines did several complicated computation
    # the computation refers to generate_random_rotation_translation in ./tools/rotation_utils.py
    # self.view_in[3:6] denotes translation which was not used in this work, so did not consider
    self.viewpoint1 = ((self.view_in[:, 0] * 180.0 / math.pi) - float(cfg['azi_low'])) / (float(cfg['azi_high']) - float(cfg['azi_low']))
    self.viewpoint2 = ((self.view_in[:, 1] * 180.0 / math.pi) - (90.0 - float(cfg['ele_high']))) / (float(cfg['ele_high']) - float(cfg['ele_low']))
    self.viewpoint3 = (self.view_in[:, 2] - float(cfg['scale_low'])) / (float(cfg['scale_high']) - float(cfg['scale_low']))
    self.viewpoint1 = tf.expand_dims(self.viewpoint1, axis=1)
    self.viewpoint2 = tf.expand_dims(self.viewpoint2, axis=1)
    self.viewpoint3 = tf.expand_dims(self.viewpoint3, axis=1)
    self.viewpoint_23 = tf.concat((self.viewpoint2, self.viewpoint3), 1)

    # since the azimuth is a loop (0 degree to 360 degree)
    # the difference should be computed as a loop (i.g., the difference between 0.1 and 0.9 is 0.2)
    tf_minimum = tf.minimum(self.viewpoint1, self.camera_given_x[:, 0])
    tf_maximum = tf.maximum(self.viewpoint1, self.camera_given_x[:, 0])
    min_loop = tf.minimum((tf_maximum - tf_minimum), (tf_minimum + 1 - tf_maximum))

    # the viewpoint fed to generator should be radian
    # so the self.camera which ranges from 0~1 should be changed to radian (except scaling)
    # self.camera[3:6] denotes translation which was not used in this work, so did not consider
    # but the viewpoint fed to generator should have 6 dimension, so tf.concat was used to add dimension
    self.rec_viewpoint1 = (self.camera[:, 0] * (float(cfg['azi_high']) - float(cfg['azi_low'])) + float(cfg['azi_low'])) * math.pi / 180.0
    self.rec_viewpoint2 = (self.camera[:, 1] * (float(cfg['ele_high']) - float(cfg['ele_low'])) + (90.0 - float(cfg['ele_high']))) * math.pi / 180.0
    self.rec_viewpoint3 = (self.camera[:, 2] * (float(cfg['scale_high']) - float(cfg['scale_low']))) + float(cfg['scale_low'])
    self.rec_viewpoint1 = tf.expand_dims(self.rec_viewpoint1, axis=1)
    self.rec_viewpoint2 = tf.expand_dims(self.rec_viewpoint2, axis=1)
    self.rec_viewpoint3 = tf.expand_dims(self.rec_viewpoint3, axis=1)
    self.rec_viewpoint = tf.concat((self.rec_viewpoint1, self.rec_viewpoint2, self.rec_viewpoint3, self.view_in[:, 3:6]), 1)


    # to reconstruct images
    self.rec_w = self.Q_c
    rec_s0, rec_b0 = self.z_mapping_function(self.rec_w, self.gf_dim * 8, 'g_z0', reuse=True)
    rec_s1, rec_b1 = self.z_mapping_function(self.rec_w, self.gf_dim * 4, 'g_z1', reuse=True)
    rec_s2, rec_b2 = self.z_mapping_function(self.rec_w, self.gf_dim * 2, 'g_z2', reuse=True)
    rec_s4, rec_b4 = self.z_mapping_function(self.rec_w, self.gf_dim * 4, 'g_z4', reuse=True)
    rec_s5, rec_b5 = self.z_mapping_function(self.rec_w, self.gf_dim, 'g_z5', reuse=True)
    rec_s6, rec_b6 = self.z_mapping_function(self.rec_w, self.gf_dim // 2, 'g_z6', reuse=True)

    self.rec_w_plus = tf.concat([rec_s0, rec_b0, rec_s1, rec_b1, rec_s2, rec_b2, rec_s4, rec_b4, rec_s5, rec_b5, rec_s6, rec_b6], 1)
    self.reconstruct = gen_func(self.rec_w_plus, self.rec_viewpoint, reuse=True)

    self.c_loss_1 = cfg["lambda_camera_pose"] * tf.reduce_mean(tf.square(min_loop))
    self.c_loss_23 = cfg["lambda_camera_pose"] * tf.reduce_mean(tf.square(self.camera_given_x[:,1:3] - self.viewpoint_23))
    self.c_loss = self.c_loss_1 + self.c_loss_23
    self.q_loss = cfg["lambda_latent"] * tf.reduce_mean(tf.square(self.Q_c_given_x - self.z))
    self.rec_loss = cfg["lambda_reconstruction"] * tf.reduce_mean(tf.square(self.reconstruct - self.inputs))
    self.d_loss = self.d_loss + self.c_loss + self.q_loss + self.rec_loss
    self.g_loss = self.g_loss + self.c_loss + self.q_loss + self.rec_loss
  ...
  def train_HoloGAN(self, config):
      ...
      # the following lines were used for plotting the loss using matplotlib library
      ...
      for epoch in range(cfg['max_epochs']):
          ...
          # decrease the learning rates gradually
          ...
          for idx in range(0, batch_idxs):
              ...
              errC = self.c_loss.eval(feed)
              err_Rec = self.rec_loss.eval(feed)

              counter += 1
              print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f, q_loss: %.8f, c_loss: %.8f, rec_loss: %.8f" \
                    % (epoch, idx, batch_idxs,
                       time.time() - start_time, errD, errG, errQ, errC, err_Rec))


              """
              # if you want to use matplotlib to plot loss, cancel this comment out
              if np.mod(counter, 100) == 1 and (not could_load):
                  pass
                  iterations = np.append(iterations, int(counter // 100))
                  D_loss = np.append(D_loss, errD)
                  G_loss = np.append(G_loss, errG)
                  Q_loss = np.append(Q_loss, 5 * errQ)
                  C_loss = np.append(C_loss, 5 * errC)
                  Rec_loss = np.append(Rec_loss, 5 * err_Rec)
                  plt.figure()
                  plt.plot(iterations, D_loss, label="D_loss")
                  plt.plot(iterations, G_loss, label="G_loss")
                  plt.plot(iterations, Q_loss, label="5*Q_loss")
                  plt.plot(iterations, C_loss, label="5*C_loss")
                  plt.plot(iterations, Rec_loss, label="5*Rec_loss")
                  plt.xlabel("Iteration")
                  plt.ylabel("Various Loss")
                  plt.legend()
              ...
              if np.mod(counter, 2000) == 1:
                  ...
                  view = self.camera_given_x.eval(feed_eval)
                  print("check the viewpoints:  ")
                  print(view[:10, :3])
  ...
  def sample_HoloGAN(self, config):
      ...
      for i in range(low, high, step):
          if config.rotate_azimuth:
              ...
              sample_view = np.tile(
                  np.array([320 * math.pi / 180.0, (90 - i) * math.pi / 180.0, 1.3, 0, 0, 0]), (cfg['batch_size'], 1))
          else:
  ...
  def sample_HoloGAN_target_image_update_tuple(self, config):
      ...
      _,_,_,_,_,_,_,_, gen_feature_h3, gen_feature_h4 = self.discriminator_IN_style_res128(generated_images, cont_dim=cfg['z_dim'], reuse=True)
...
#=======================================================================================================================
  ...
  def discriminator_IN_style_res128(self, image,  cont_dim, reuse=False):
      ...
      with tf.variable_scope("discriminator") as scope:
          ...
          #print("================================================================================")
          #print("discriminator_h4:  ", h4.get_shape().as_list())

          #Returning logits to determine whether the images are real or fake
          h5 = linear(slim.flatten(h4), 1, 'd_h5_lin')

          # Recognition network for latent variables has an additional layer
          encoder = lrelu((linear(slim.flatten(h4), 200, 'd_latent')))
          cont_vars = linear(encoder, cont_dim, "d_latent_prediction")

          camera_pose_encoder1 = lrelu((linear(slim.flatten(h4), 200, 'd_camera_pose1')))
          camera_pose_encoder2 = lrelu((linear(slim.flatten(camera_pose_encoder1), 50, 'd_camera_pose2')))
          cont_camera_pose = linear(camera_pose_encoder2, 3, "d_camera_pose_prediction")

          return tf.nn.sigmoid(h5), h5, tf.nn.tanh(cont_vars), tf.nn.sigmoid(cont_camera_pose), d_h1_logits, d_h2_logits, d_h3_logits, d_h4_logits, h3, h4
  ...
  def generator_AdaIN_res128(self, w_plus, view_in, reuse=False):
      ...
      counter = 0
      s0, b0 = w_plus[:, counter: self.gf_dim * 8], w_plus[:,
                                                    (counter + self.gf_dim * 8): (counter + self.gf_dim * 8 * 2)]
      counter = counter + self.gf_dim * 8 * 2
      #print("check_after_s0:   ", counter)
      s1, b1 = w_plus[:, counter: (counter + self.gf_dim * 4)], w_plus[:, (counter + self.gf_dim * 4): (
                  counter + self.gf_dim * 4 * 2)]
      counter = counter + self.gf_dim * 4 * 2
      #print("check_after_s1:   ", counter)
      s2, b2 = w_plus[:, counter: (counter + self.gf_dim * 2)], w_plus[:, (counter + self.gf_dim * 2): (
                  counter + self.gf_dim * 2 * 2)]
      counter = counter + self.gf_dim * 2 * 2
      #print("check_after_s2:   ", counter)
      s4, b4 = w_plus[:, counter: (counter + self.gf_dim * 4)], w_plus[:, (counter + self.gf_dim * 4): (
                  counter + self.gf_dim * 4 * 2)]
      counter = counter + self.gf_dim * 4 * 2
      s5, b5 = w_plus[:, counter: (counter + self.gf_dim)], w_plus[:,
                                                            (counter + self.gf_dim): (counter + self.gf_dim * 2)]
      counter = counter + self.gf_dim * 2
      s6, b6 = w_plus[:, counter: (counter + self.gf_dim // 2)], w_plus[:, (counter + self.gf_dim // 2): (
                  counter + self.gf_dim // 2 * 2)]
      """
      print("==================================================================================")
      ...
      tf.print("check s6, b6 shape:  ", s6.shape, "  ", b6.shape)
      """
